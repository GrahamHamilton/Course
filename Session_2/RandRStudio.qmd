...
Github packages are usually named after the repository name then the package name e.g. grahamhamilton/Rpackage
...
expression_table <- pivot_real_data %>%
  ...
  filter(Gene_names == "CHAC1_1279")
```

```{r}
ggplot(expression_table, aes(x = Sample, y = Expression, fill = Sample)) +
  geom_boxplot() +
  scale_y_log10() +
  labs(x = NULL,
       y = "Log10(Expression)") +
