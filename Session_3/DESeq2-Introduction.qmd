...
```{r}
sample_table <- read_tsv("data/SampleDescription.tsv")
sample_table  %>%
  head(n = 6) %>%
  kbl() %>%
  kable_styling() 
...
```{r}
time  %>%
  select(c(-Expression,-Mean)) %>%
  mutate(Batch = unlist(str_split(rep(strrep(c("A", "B", "C"), 5),2),""))) %>%
  head(n = 10) %>%
  kbl() %>%
  kable_styling()
```
...
#| echo: true
#| eval: false
model.matrix(~ Batch + Condition + Time + Condition:Time, sample_table)
```
...
#| echo: true
#| code-line-numbers: "1"
sample_table <- read_tsv("./data/SampleDescription.tsv",col_types = "ff")
sample_table  %>%
  head(n = 6) %>%
  kbl() %>%
  kable_styling() 
...
files <- list.files("./data/htseq_counts/") %>%
  ...
  as.data.frame %>%
  rename(files = ".") %>% 
  bind_cols(read_tsv("./data/SampleDescription.tsv",col_types = "cf"))
